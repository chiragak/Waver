...
if(isset($email_id))
{
	$result = mysqli_query($conn,"SELECT * FROM login where name='" . $_POST['name'] . "'");
    $count  = mysqli_num_rows($result);
    if($count){
		$location="loginpage.php";
	      header("Location: $location?userexist=success");
           exit();
	}
    $result1 = mysqli_query($conn,"SELECT * FROM login where gmail='" . $_POST['email'] . "'");
